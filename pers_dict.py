def registroEstudiantes():
    ...
    if estudiantes:
        id_actual_est = estudiantes[-1]["id"] + 1
    else:
        id_actual_est = 1
    ...
    with open("estudiantes.txt", "w", encoding="utf-8") as archivo:
        for est in estudiantes:
            ...
            for mat in est["materias"]:
                notas_str = "-".join(str(n) for n in mat["notas"])
                materias_txt.append(f"{mat['materia']}:{notas_str}-{mat['promedio']}")
            
            materias_final = ";".join(materias_txt)

            linea = f"{est['id']}|{est['nombre']}|{est['promedio']}|{est['aprobado']}|{materias_final}\n"
    ...
    try:
        with open("estudiantes.txt", "r", encoding="utf-8") as archivo:
            for linea in archivo:
                ...
                aprobado = datos[3] == "True"

                materias_raw = datos[4].split(";")
                materias = []

                for mat in materias_raw:
                    nombre_mat, resto = mat.split(":")
                    
                    partes = resto.split("-")
                    *notas_str, promedio_mat = partes
...
def guardar_materias(materias):
    with open("materias.txt", "w", encoding="utf-8") as archivo:
        for materia in materias:
            archivo.write(materia + "\n")
            
def leer_materias():
    try:
        with open("materias.txt", "r", encoding="utf-8") as archivo:
            materias = [linea.strip() for linea in archivo if linea.strip() != ""]
            return materias
    except FileNotFoundError:
        return ["Matemáticas", "Lengua", "Ciencias sociales", "Ciencias naturales", "Educación física", "Inglés", "Informática"]
